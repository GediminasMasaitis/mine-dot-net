...
namespace MineDotNet.GUI.Controls.Charts
{
    ...
    internal static class ChartRegistry
    {
        public static IReadOnlyList<ChartDescriptor> All { get; } = new List<ChartDescriptor>
        {
            ...
            new ChartDescriptor("Win rate surface 3D (2D sweep)",    () => new WinRateSurface3DChart()),
            new ChartDescriptor("Avg time surface 3D (2D sweep)",    () => new AvgTimeSurface3DChart()),
            new ChartDescriptor("Avg iterations surface 3D (2D sweep)", () => new AvgIterationsSurface3DChart()),
